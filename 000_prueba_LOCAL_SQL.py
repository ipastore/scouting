...
with st.form('Agregar Jugador'):
    ...
    if posicion == 'Extremo':
        ...
        despliegue = st.number_input('Despliegue', min_value=0, max_value=100, step=1, value=None)
        remate = st.number_input('Remate', min_value=0, max_value=100, step=1, value=None)
        regate = st.number_input('Regate', min_value=0, max_value=100, step=1, value=None)
        centros = st.number_input('Centros', min_value=0, max_value=100, step=1, value=None)
...
## FALTA CACHEAR EL CAMBIO DE POSICION PORQUE HAY QUE BORRAR LA POSICION ANTERIOR
    with st.form('Actualizar Jugador'):
            ...
            nuevo_categoria = st.number_input('Nueva Categoría', min_value=1900, max_value=2100, step=1, value=int(current_categoria) if math.isnan(current_categoria) == False else None)
            ...
            nueva_altura = st.number_input('Nueva Altura (m)', min_value=0.00, step=0.01, value=float(current_altura) if current_altura is not None and math.isnan(current_altura) == False else 0.00)
            nuevo_peso = st.number_input('Nuevo Peso (kg)', min_value=0, step=1, value=int(current_peso) if current_peso is not None and math.isnan(current_peso) == False else 0)
            ...
            nuevo_valor_transfermarket = st.number_input('Nuevo Valor Transfermarket (MM USD)', min_value=0.00, step=0.01, value=float(current_valor_transfermarket))
